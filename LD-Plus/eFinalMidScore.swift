...
class eFinalMidScore: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Sem \(semNumber)"
        print(dept)
        tableview.dataSource = self
        tableview.delegate = self
        print("eroll:-\(enroll_number)")
        let my_domain = "http://"
        let my_path = "jarvis/midResult.php"
                ...
                if( parseJSON["status"]! as! String == "Success")
                {
                    ...
                    for i in 1  ..< self.n 
                    {
                        ...
                        if(parseJSON[a] as? String == nil){
                            self.tableData.append("Result Pending :)");
                        }
                        else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "eFInalMidScore", for: indexPath) as! DataCell
